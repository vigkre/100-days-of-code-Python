...
class DataManager:
    ...
    def __init__(self):
        ...
        self.sheety_endpoint = "https://api.sheety.co/8b6fb5685aeb23411e17e5b6231062ff/python100DaysFlightDeal/prices"


    def get_price_data_from_sheet(self):
        """GET API to get the flight price data from the google sheets."""
        response = requests.get(url=self.sheety_endpoint, auth=self._authorization, verify=False).json()        
    ...
    def update_iata_code(self):
        for city in self.destination_data:
            put_sheety_endpoint = f"{self.sheety_endpoint}/{city['id']}"        
            new_data = {
                "price": {
